...
class InputSystem {
  ...
  bool handleTapDown(TapDownEvent event) {
    ...
    if (_handleObstacleTap(localPosition)) {
      return true;
    }
  ...
  bool _handleObstacleTap(Vector2 tapPosition) {
  ...
  /// Convert screen coordinates to game world coordinates
  Vector2 _screenToGameCoordinates(Vector2 screenPosition) {
  ...
  double _calculateTapRadius(BaseObstacle obstacle) {
    final baseRadius = (obstacle.obstacleSize.width + obstacle.obstacleSize.height) / 4;
  ...
  List<BaseObstacle> getObstaclesAtPoint(Vector2 point) {
    final gamePosition = _screenToGameCoordinates(point);
    return game.obstacles.where((obstacle) => 
        _isPointInObstacle(gamePosition, obstacle)).toList();
  }

  /// Check if any obstacle is being touched (for UI state)
  bool isAnyObstacleBeingTouched(Vector2 point) {
